...
def test_with_graphql_data():
    ...
    # 正確なセクション抽出
    outside_sections = []
    for match in re.finditer(r"<summary>⚠️ Outside diff range comments \((\d+)\)</summary><blockquote>(.*?)</blockquote></details>", body, re.DOTALL):
    ...
    for i, (count, content) in enumerate(outside_sections, 1):
        ...
        # ファイルブロック数確認
        file_matches = list(re.finditer(r"<details>\s*<summary>([^<]+?)\s*\((\d+)\)</summary><blockquote>(.*?)</blockquote></details>", content, re.DOTALL))
        ...
        for file_match in file_matches:
            ...
            # 実際のコメント数を確認
            comment_matches = list(re.finditer(r"`([^`]+)`:\s*\*\*(.*?)\*\*", file_content, re.DOTALL))
