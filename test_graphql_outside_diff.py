...
def get_pr_review_comments_graphql(owner: str, repo: str, pr_number: int) -> Dict[str, Any]:
    ...
    for review in reviews:
        ...
        for comment in comments:
            ...
            if position is None and line is None:
                print(f"    🔍 位置情報なしコメント発見: {comment_author} - {len(comment_body)}文字")
...
def main():
    ...
    print(f"✅ GITHUB_TOKEN確認: {token[:10]}...{token[-4:] if len(token) > 14 else '***'}")
