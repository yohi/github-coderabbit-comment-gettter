            ...
            with urllib.request.urlopen(request) as response:
                if response.status == 200:
                    ...
                    if not result.get('data', {}).get('repository', {}).get('pullRequest'):
                        logger.error("GraphQL レスポンスにプルリクエストデータが含まれていません")
                        break
                    ...
                    for thread in threads:
                        if thread['isResolved']:
                            ...
                            if thread['comments']['pageInfo']['hasNextPage']:
                                logger.warning("スレッド内に50を超えるコメントがあります。一部取得されていない可能性があります。")
                    
                    total_resolved += page_resolved
                    logger.debug(f"ページ {page_count}: {len(threads)} スレッド, {page_resolved} 解決済みコメント")
                    ...
                    response_text = response.read().decode('utf-8') if hasattr(response, 'read') else 'Unknown error'
                    logger.error(f"GraphQL API エラー: HTTP {response.status} - {response_text[:200]}")
                    break
                    
        except Exception as e:
            logger.warning(f"GraphQL API呼び出しでエラー (ページ {page_count}): {str(e)}")
            break
    
    logger.info(f"GraphQL API完了: {page_count}ページ処理, {len(resolved_ids)} 件の解決済みコメントを検出")
    return resolved_ids
...
def run_lightweight_mode(args) -> int:
    ...
    output.append("  -d '{")
    output.append("    \"body\": \"@coderabbitai 対応不要：[技術的根拠を記載]。適切と判断される場合は**この特定の課題のみ**を解決済みにしてください。他の課題は変更しないでください。\",")
    ...
    output.append("  -H \"Content-Type: application/json\" \\\\")
    output.append("  -d '{")
    output.append("    \"body\": \"@coderabbitai この指摘は妥当ですが、現在のPhase/ステップでは対応対象外です。現在: [具体的なPhase名]、対応予定: [具体的な将来Phase名]。**記憶依頼**: 以下を構造化記録し『[将来Phase名]』開始時・[技術領域]作業時に積極的に思い出してください - 指摘:[要約] 対象:[ファイル:行数] 解決方法:[実装案] 優先度:[高/中/低] 思い出し条件:[具体的なトリガー]。適切と判断される場合は**この特定の課題のみ**を解決済みにしてください。他の課題は変更しないでください。\",")
    output.append("    \"in_reply_to\": [COMMENT_ID]")
    output.append("  }'")
    output.append("```")
