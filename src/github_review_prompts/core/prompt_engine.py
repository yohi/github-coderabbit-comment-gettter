...
```"""
    ...
    def generate_main_prompt(self, 
        ...
        # 簡潔なメインプロンプト
        prompt_parts.append(f"""# CodeRabbit レビュー対応 - 実行プロンプト

## 🎯 基本方針  
あなたはシニアエンジニアとして、**技術的正確性を最優先**に判断してください。
...
## ⚡ 3段階優先度システム
🔴 **緊急**: セキュリティ・バグ・破綻リスク → 即座対応必須
🟡 **重要**: 機能改善・リファクタリング → PR内対応推奨  
🟢 **任意**: スタイル・軽微な改善 → 余裕があれば対応

## 作業手順

### Phase 1: 分析・計画（15-20分）
1. **全コメント確認** → 🔴🟡🟢で分類
2. **TODOリスト作成** → 🔴緊急から順序付け

### Phase 2: 実行（40-60分、🔴→🟡→🟢順）
3. **🔴緊急項目を最優先処理**
4. **各修正後に検証実行**：
   - 構文チェック: `python -m py_compile ファイル名`
   - Lint実行: `ruff check ファイル名`  
   - セキュリティスキャン: トークン漏洩なし確認
5. **❌⏳🤔判定時はcurl返信必須**

## 🔄 エラー時の具体的対応
### 修正失敗時のロールバック手順
1. **構文エラー**: `git checkout HEAD -- ファイル名` でロールバック
2. **判断不明**: 🤔要確認で一旦保留、保守的対応選択  
3. **修正失敗**: `git stash` で変更保存、理由記録
4. **判断に迷う場合**: セキュリティ > 機能性 > 可読性の順で優先度判断

### 🗣️ 返信テンプレート
**❌ 対応不要**: `@coderabbitai [技術的根拠]により対応不要と判断します。この課題のみを解決済みにしてください。`
**⏳ 将来対応**: `@coderabbitai この指摘は妥当ですが、現在のフェーズでは対応しません。[将来のフェーズ名]で対応予定です。この課題のみを解決済みにしてください。`
**🤔 要確認**: `@coderabbitai [確認したい内容]について詳細説明をお願いします。`

## ✅ 検証チェックリスト
各修正後に以下を実行：
- [ ] 構文エラーなし: `python -m py_compile ファイル名`
- [ ] Lintエラー解消: `ruff check ファイル名`
- [ ] セキュリティスキャン通過: トークン漏洩検出なし
- [ ] 必要な curl返信実行済み（❌⏳🤔判定時）
