...
### 作業中のリマインダー
- 各TODO完了時: 「残りX件」を必ず表示
- セキュリティ項目完了時: 「Git操作を忘れずに」を表示
- 最後のTODO完了時: 「⚠️ Git・返信・報告を忘れずに実行」を表示

### 完了判定基準
**以下が全て完了するまで作業終了しない**:
1. 全TODOの処理（✅/❌/⏳/🤔のいずれかに分類）
2. 必要な返信の完了
3. Git操作の完了（コミット・プッシュ）
4. 結果報告の出力
...
**作業完了後、以下のフォーマットで必ず報告してください：**

---
# 🎯 CodeRabbitレビューコメント対応完了報告
...
## 💬 返信実行状況
- [ ] ❌対応不要返信: X件 → 全て完了/一部未完了
- [ ] ⏳将来対応返信: X件 → 全て完了/一部未完了  
- [ ] 🤔要確認返信: X件 → 全て完了/一部未完了
...
**コミットメッセージ**: "CodeRabbitレビューコメント対応"
**変更ファイル一覧**:
- [ファイル1]
- [ファイル2]
