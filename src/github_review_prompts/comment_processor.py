...
from .utils.parsers import extract_ai_agent_prompt, categorize_prompt, determine_priority
...
class CommentProcessor:
        ...
        for comment in comments:
            try:
                ...
                if ai_prompt:
                    ...
                    self.logger.debug(f"AI プロンプト抽出成功: コメント ID {comment_id}")
                
            except Exception as e:
                error_msg = f"コメント処理エラー (ID: {comment.get('id', 'unknown')}): {str(e)}"
    ...
    def _create_review_comment(self, comment: Dict[str, Any], body: str, is_resolved: bool) -> ReviewComment:
    ...
    def _extract_comment_context(self, comment: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ファイル情報
        if comment.get("path"):
            context["file_extension"] = comment["path"].split(".")[-1] if "." in comment["path"] else ""
            context["directory"] = "/".join(comment["path"].split("/")[:-1]) if "/" in comment["path"] else ""
        ...
        if user_info:
            ...
            context["is_coderabbit"] = "coderabbitai" in user_info.get("login", "").lower()
    ...
    def _calculate_line_range(self, comment: Dict[str, Any]) -> Optional[Dict[str, int]]:
        """コメントの行範囲を計算"""
        line = comment.get("line")
        original_line = comment.get("original_line")
    ...
    def filter_prompts_by_criteria(
        ...
        if categories:
            ...
            self.logger.debug(f"カテゴリフィルター適用後: {len(filtered_prompts)} プロンプト")
        
        # 優先度フィルター
        if priorities:
            filtered_prompts = [p for p in filtered_prompts if p.priority in priorities]
            self.logger.debug(f"優先度フィルター適用後: {len(filtered_prompts)} プロンプト")
        
        # 作成者フィルター
        if authors:
            filtered_prompts = [p for p in filtered_prompts if p.author in authors]
            self.logger.debug(f"作成者フィルター適用後: {len(filtered_prompts)} プロンプト")
        
        # ファイルパターンフィルター
        if file_patterns:
            filtered_prompts = []
            for prompt in filtered_prompts:
                if any(self._match_file_pattern(prompt.file_path, pattern) for pattern in file_patterns):
                    filtered_prompts.append(prompt)
            self.logger.debug(f"ファイルパターンフィルター適用後: {len(filtered_prompts)} プロンプト")
    ...
    def sort_prompts(
        ...
        if sort_by == "priority":
            ...
            return sorted(
                prompts, 
                key=lambda p: p.line_number or 0, 
                reverse=reverse
            )
        elif sort_by == "author":
    ...
    def get_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                if self.stats.unresolved_comments > 0 else 0
            ),
            "errors": self.stats.errors
        }
    
    def validate_comment_data(self, comments: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
        """コメントデータの基本的な検証"""
        validation_errors = []
        ...
        for i, comment in enumerate(comments):
            ...
            for field in required_fields:
                if field not in comment:
                    validation_errors.append(f"コメント {i}: 必須フィールド '{field}' が見つかりません")
