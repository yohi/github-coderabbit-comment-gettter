...
出力:
  ...
  - コンソール出力
    ...
    output.append("**前提条件**: ")
    output.append("```bash")
    output.append("export GITHUB_TOKEN=\"your_github_token_here\"")
    output.append("```")
    output.append("")
    output.append("#### ❌ 対応不要の場合")
    ...
    output.append("```")
    output.append("// TODO: CodeRabbit指摘 - [技術的根拠により対応不要] - [YYYY-MM-DD]")
    output.append("```")
    output.append("")
    ...
    output.append("**ソースコード修正**: 指摘箇所に以下のTODOコメントを追加")
    output.append("```")
    output.append("// TODO: CodeRabbit指摘間違い - [理由] - [YYYY-MM-DD]")
    output.append("```")
    output.append("")
    output.append("**使用方法**:")
    ...
    output.append("4. **対応不要・指摘間違いの場合**: 該当ソースファイルにTODOコメントを追加")
