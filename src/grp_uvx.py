...
3. **対応判断**: 各コメントに対して以下のいずれかの対応を決定してください：
   ...
   - ❌ 対応不要（技術的に間違っているか不適切）
   - 🤔 要確認（追加情報が必要）
   - ⏳ 将来対応（現在のPhase/ステップでは対応しないが記憶する）
...
5. **将来対応の場合**: @coderabbitaiに「将来のPhase/ステップで対応予定」の旨を返信し、記憶を依頼
6. **重要**: 各対応完了後は必ず該当コメントの解決済み指示を含める

## 出力フォーマット
各コメントに対して以下の形式で回答してください：

**コメント [番号]: [要約]**
- 判断: [✅修正実施/❌対応不要/⚠️指摘間違い/🤔要確認/⏳将来対応]
- 理由: [技術的根拠]
- 対応: [具体的な行動]
- 返信: [curl返信が必要な場合のみ]
...
**⏳ 将来対応**: `@coderabbitai この指摘は妥当ですが、現在のPhase/ステップでは対応対象外です。開発段階: [現在のPhase]、対応予定: [将来のPhase]。この指摘内容をメモリに記憶し、該当Phase/ステップで再提起してください。この課題のみを解決済みにしてください。`
...
出力:
  ...
  - コンソール出力
    ...
    output.append("  -H \"Content-Type: application/json\" \\\\")
    output.append("  -d '{")
    output.append("    \"body\": \"@coderabbitai 将来対応予定：このフェーズでは対応しませんが、[次のフェーズ/バージョン]で対応予定です。適切と判断される場合は**この特定の課題のみ**を解決済みにしてください。他の課題は変更しないでください。\",")
    output.append("    \"in_reply_to\": [COMMENT_ID]")
    output.append("  }'")
    output.append("```")
    ...
    output.append("4. **📅 将来対応予定の場合のみ**: 該当ソースファイルにTODOコメントを追加")
    output.append("5. curlコマンドを実行")
    output.append("")
