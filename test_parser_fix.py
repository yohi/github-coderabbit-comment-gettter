...
def extract_outside_diff_comments_fixed(comment_body: str) -> list:
    ...
    outside_diff_pattern = r"<summary>⚠️ Outside diff range comments \((\d+)\)</summary><blockquote>(.*)"
    ...
    print(f"📊 デバッグ: Outside diff commentsセクション発見 - 期待件数: {expected_count}")  # デバッグ用
            ...
            extracted_comments.append(
                {
                    ...
                    "line": line_info,
                    "title": title,
                    "content": content_after_title[:100] + "..." if len(content_after_title) > 100 else content_after_title,
                }
            )
...
"""
    ...
    for i, comment in enumerate(extracted, 1):
        print(f'{i}. ファイル: {comment["file_path"]} | 行: {comment["line"]} | タイトル: {comment["title"]}')
