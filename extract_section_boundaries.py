...
def find_section_boundaries():
    ...
    # GraphQLレスポンスファイルの安全な読み込み
    filename = "pr12_graphql_response.json"
    try:
    ...
    # 1. Outside diff range commentsの開始位置（正規表現で対応）
    import re

    outside_pattern = r">\s*<summary>\s*⚠️\s*Outside\s+diff\s+range\s+comments"
    outside_match = re.search(outside_pattern, body)
    outside_start = outside_match.start() if outside_match else -1
