...
def test_basic_functionality():
    ...
    result = run_command(
        "uv run grp --debug https://github.com/yohi/terraform/pull/98", timeout=120
    )
    ...
    if success:
        ...
        for line in stdout.split("\n"):
            if "返信要件分析完了" in line:
                ...
                try:
                    ...
                    stats["replies_required"] = int(
                        parts.split("返信必要=")[1].split(",")[0]
                    )
                    stats["replies_not_required"] = int(
                        parts.split("返信不要=")[1].split(",")[0]
                    )
                    stats["estimated_time"] = int(
                        parts.split("推定時間=")[1].split("分")[0]
                    )
        ...
        tests.append(
            {"name": "実際のPR処理", "success": False, "details": result["stderr"]}
        )
...
def test_new_features():
    ...
    result = run_command(
        "uv run grp --debug https://github.com/yohi/terraform/pull/98 2>&1 | grep -E '(スマートフィルタリング|フィルタ除外)'"
    )
    tests.append(
        {
            "name": "スマートフィルタリング",
            "success": result["success"],
            "details": (
                "フィルタリング機能動作中" if result["success"] else "ログ出力なし"
            ),
        }
    )
    ...
    print("2. 返信判定マトリックス...")
    result = run_command(
        "uv run grp --debug https://github.com/yohi/terraform/pull/98 2>&1 | grep -E '(返信判定|返信要件分析)'"
    )
    tests.append(
        {
            "name": "返信判定マトリックス",
    ...
    print("4. 使用状況メトリクス...")
    try:
        from github_review_prompts.utils.usage_metrics import (
...
def test_performance_regression():
    ...
    for i in range(3):
        ...
        start_time = time.time()
        result = run_command(
            "uv run grp https://github.com/yohi/terraform/pull/98 > /dev/null 2>&1"
        )
        end_time = time.time()
...
def generate_validation_report(all_tests):
    ...
    report = {
        ...
        "passed_tests": sum(
            sum(1 for test in tests if test["success"]) for tests in all_tests.values()
        ),
        "failed_tests": sum(
            sum(1 for test in tests if not test["success"])
    ...
    report_file = (
        project_root
        / f"validation_report_v2.0.0_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
