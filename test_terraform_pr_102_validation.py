...
def test_terraform_pr_102_output():
    ...
    prompt = engine.generate_main_prompt(
        comments=comments, pr_info=pr_info, github_token="mock_token"
    )
    ...
    expected_patterns = [
        ...
        ("🔑 作業開始前の必須確認", "環境設定確認"),
        ("GITHUB_TOKEN環境変数の確認", "トークン確認指示"),
        ("echo $GITHUB_TOKEN", "トークン確認コマンド"),
        ("もしトークンが表示されない場合", "トークン設定手順"),
        ...
        (
            "https://api.github.com/repos/yohi/terraform/pulls/102/comments",
            "PR特有のURL",
        ),
        ...
        ("Phase別実施状況", "Phase別報告"),
        ("Git操作実行状況（詳細）", "Git操作詳細報告"),
        ("コミット実行状況", "コミット詳細"),
        ("プッシュ実行状況", "プッシュ詳細"),
        ("git log --oneline", "Git確認コマンド"),
        ("コメント返答実行状況（詳細）", "コメント返答詳細"),
        ("返答実行サマリー", "返答サマリー"),
        ("実行したcurlコマンド記録", "curl実行記録"),
        ("残対応状況・継続作業計画（詳細）", "残対応詳細"),
        ("作業完了状況サマリー", "完了状況サマリー"),
        ("作業状態", "作業状態判定"),
        ("緊急項目未完了", "緊急未完了"),
        ("次回作業計画", "継続計画"),
...
def test_comment_classification_accuracy():
    ...
    for comment_body, expected_type, description in test_cases:
        ...
        if actual_type == expected_type:
            ...
            print(
                f"❌ {description}分類: 失敗 (期待: {expected_type}, 実際: {actual_type})"
            )
