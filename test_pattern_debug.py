...
def debug_outside_diff_extraction():
    ...
    try:
        ...
        print(f"❌ 期待されるデータ構造が見つかりません: {e}")
        print(
            f"実際のキー: {list(data.keys()) if isinstance(data, dict) else 'データが辞書ではありません'}"
        )
        return
    except IndexError:
        print("❌ レビューノードが存在しません")
        return
    ...
    for i, pattern in enumerate(patterns_to_test, 1):
        ...
        if match:
            ...
            print(
                f"✅ マッチ成功: 期待件数={expected_count}, コンテンツ長={len(content)}文字"
            )
