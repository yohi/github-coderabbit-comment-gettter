...
def analyze_outside_diff_from_graphql():
    ...
    for i, review in enumerate(reviews, 1):
        ...
        if outside_matches:
            print(
                f"✅ Outside diff range commentsセクション発見: {len(outside_matches)}個"
            )
            ...
            for j, match in enumerate(outside_matches, 1):
                ...
                file_matches = list(
                    re.finditer(file_pattern, outside_content, re.DOTALL)
                )
                ...
                for file_match in file_matches:
                    ...
                    comment_matches = list(
                        re.finditer(comment_pattern, file_content, re.DOTALL)
                    )

                    print(
                        f"  📁 {file_path}: 期待{file_comment_count}件, 実際{len(comment_matches)}件"
                    )
                    total_extracted += len(comment_matches)

                print(
                    f"✅ セクション{j}の総抽出件数: {total_extracted}/{expected_count}"
                )
        else:
        ...
        if review["author"]["login"] in ["coderabbitai", "coderabbitai[bot]"]:
            ...
            if extracted:
                ...
                for i, comment in enumerate(extracted[:5], 1):  # 最初の5件のみ表示
                    print(
                        f"{i}. {comment['file_path']} - {comment['line']} - {comment['title']}"
                    )
